...
#
# for only 2016 docs
# python ensemble2newDocs.py TrumpTexts 1 signalOutput-coco_3_cv_3_netAng_30_twc_20_tfidfNoPro_pronounFrac_bin_10-7B7C2E.csv auto
#
# for even distribution of new and old groups 
# python ensemble2newDocs.py TrumpTexts 1 signalOutput-coco_3_cv_3_netAng_30_twc_20_tfidfNoPro_pronounFrac_bin_10-2BRVHT.csv auto
# OR
# python ensemble2newDocs.py TrumpTexts 1 signalOutput-coco_3_cv_3_netAng_30_twc_20_tfidfNoPro_pronounFrac_bin_10-Z10HBZ.csv auto
#
# 
#########
...
def textAnalysis(paramList):
    ...
    print('$$$$$$$$$$$$$\nSTARTING ' + str(groupId) + str(subFileList))
    ...
    try:
        ...
        print('!*!*!*!*!\nDSM FAILURE\n!*!*!*!*!')
        with open('DSM-FAILED-DOCUMENTS.txt', "a") as text_file:
            text_file.write(str(subFileList) + '\n')
        return(['_'.join(groupId + subFileList)]+[len(subFileList)]+[None] * 17)   
    ...
    return(['_'.join(groupId + subFileList)]+[len(subFileList),timeRun]+[keywordPicks]+sentimentList+[PSJudge]+judgementAvg+pronounCounts+[avgSD]+[avgEVC])   
...
if __name__ == '__main__':
    ...
    #
    signalDF.to_csv(writeDirectory+'TESTsignalDF.csv', encoding = 'utf-8') 
    print('WROTE TESTsignalDF.csv TO FILE (overwritten each time)')
    
    print(signalDF.shape)
    #drop any rows with NA values
    signalDF=signalDF.dropna(axis=0)
    print(signalDF.shape)
    signalDF.to_csv(writeDirectory+'TESTpostNAsignalDF.csv', encoding = 'utf-8')
    ...
    for cv1 in cv:
        print('%%%%%%%%%%')
        print(cv1)
        signalTestDF = signalDF.loc[cv1]
        cv1trainInd = [x for x in shuf if x not in cv1]
        signalTrainDF = signalDF.loc[cv1trainInd]
